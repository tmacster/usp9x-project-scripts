...
usp9x_filt <- filter(usp9x, chr != "chrX" & chr != "chrY") %>% select(chr, start, end, inp_a, neg_a, neg_b, low_8a, low_8b, high_8a, high_8b) %>% filter_all(any_vars(. != 0)) %>% filter_at(vars(neg_a:high_8b), all_vars(. > 1))
...
usp9x_norm <- usp9x_norm %>% transmute(neg_a = neg_a/inp_a, neg_b = neg_b/inp_a, low_8a = low_8a/inp_a, low_8b = low_8b/inp_a, 
  ...
  select(-starts_with("inp")) %>% add_column(chr = usp9x_filt$chr, start = usp9x_filt$start, end = usp9x_filt$end, .before = 1)


# now van Mierlo data
vm <- read_tsv("vm_raw_counts.tab", skip = 1, col_names = c("chr", "start", "end", "input_2i", "rep1_2i", "rep2_2i", "input_fbs", "rep1_fbs", "rep2_fbs"))
vm_filt2 <- filter(vm, chr != "chrX" & chr != "chrY") %>% filter_all(all_vars(. != 0)) %>% filter_at(vars(matches("rep")), all_vars(. > 1))

vm_spike <- c(1, 0.74, 0.70, 1, 0.43, 0.45)
vm_norm <- vm %>% do(sweep(vm_filt2[,-c(1:3)], 2, vm_spike, "*")) 
vm_norm <- vm_norm %>% transmute(rep1_2i = rep1_2i/input_2i, rep2_2i = rep2_2i/input_2i, rep1_fbs = rep1_fbs/input_fbs, rep2_fbs = rep2_fbs/input_fbs) %>% select(-starts_with("inp")) %>% mutate(chr = vm_filt2$chr, start = vm_filt2$start, end = vm_filt2$end)
...
embryo_filt <- select(embryo, -starts_with("MII"), -starts_with("zyg")) %>% filter(chr != "chrX" & chr != "chrY") %>% filter_all(all_vars(. != 0)) %>% filter_at(vars(-contains("input")), all_vars(. > 1))

##########
# only through E7.5
#  based on *deduplicated* bam file size...
scale <- c(32955958, 28259537, 30329242, 24293121, 25977939, 16167853, 17860721, #e6.7-e7.5
  #11273427, 18745801, 17767007, 13931413, #zygote
  ...
  #14630835, 10860647, 17316006, 17423438, #MII
  8328612, 18551283, 19835285, 11701194, 16621536, 17918080)/1000000 #morula, TE

embryo_norm <- embryo_filt %>% do(sweep(embryo_filt[,-c(1:3)], 2, scale, "/")) %>% 
  transmute(#zygote_rep1 = zyg_rep1/zyg_input, zygote_rep2 = zyg_rep2/zyg_input, zygote_rep3 = zyg_rep3/zyg_input, MII_rep1 = MII_rep1/MII_input, MII_rep2/MII_input, MII_rep3/MII_input,
            rep1_2c = rep1_2c/inp_2c, rep2_2c = rep2_2c/inp_2c, rep3_2c = rep3_2c/inp_2c, rep1_4c = rep1_4c/inp_4c, rep2_4c = rep2_4c/inp_4c, rep3_4c = rep3_4c/inp_4c, 
            ...
            mor_rep1 = mor_rep1/mor_input, mor_rep2 = mor_rep2/mor_input, TE_rep1 = TE_rep1/TE_input, TE_rep2 = TE_rep2/TE_input,
...
usp9x_embryo_vm <- inner_join(usp9x_norm, embryo_norm, by = c("chr", "start", "end")) %>% inner_join(vm_norm, by = c("chr", "start", "end")) %>% select(-chr:-end, -matches("neg"))
usp9x_embryo_vm <- select(usp9x_embryo_vm, -matches("TE")) # removes TE samples
